...
- `/pushall`: process all doctrine-vendoring workspace repos in order (`inspr-modules` → `nixcfg` → `inspr` → `fleetcom`) — handles doctrine submodule bump propagation; **no confirmation**, just do it. OpenClaw workspaces (`oc-workspace-*`) and other personal projects are out of scope — use `/push` per repo for those.

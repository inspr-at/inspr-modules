...
| AGENTS-AGENT-OPENCLAW-OPS.md        | `agent:openclaw-ops`  | 12    | (no slash command; overlay remains)         | OPENCLAW-OPS role overlay — Open Clearance Workspace.                                                    |
...
|---|---|---|
| [AGENTS-VERSIONING.md](AGENTS-VERSIONING.md) | Creating, publishing, consuming, comparing, pinning, deploying, or migrating any version-bearing artifact | INSPR Calendar Version v2 (`YYMMDDhhmmss.0.0`, SemVer-syntactic and fixed-width sortable; v1 `YY.MM.DD[.hh.mm.ss]` superseded), display weights and the explicit display design revision for UIs (data: `lib/calendar-version-display.json`), gradual per-repository adoption, mixed-era behavior, supply-chain gates, ecosystem exceptions, and the value-free estate inventory. |

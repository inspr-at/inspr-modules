...
```nix
{
  inputs = {
    ...
    inspr-modules.url = "github:inspr-at/inspr-modules/v0.4.4";
...
| `work-attribution-doctrine` | Keeps the always-on kernel, non-Claude mirror, full reference and product-gauntlet dispatcher aligned on ticket-first work authorization, actual session UUID attribution, single-tracker routing and value-free markers. |
...
**v0.4.4.** Extracted from a working NixOS + Home Manager fleet on 2026-05-02
...
git -C doctrine checkout v0.4.4
